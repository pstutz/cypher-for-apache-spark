...
import org.opencypher.okapi.api.value.CypherValue.{CypherList, _}
...
object CAPSTestValues {
  ...
  implicit lazy val NODE_valueGroups: ValueGroups[CypherValue] = Seq(
    Seq[CypherValue](
      CAPSNode(1L, Set("Person"), CypherMap("a" -> 1, "b" -> CypherNull)),
      CAPSNode(1L, Set("Person"), CypherMap("a" -> 1.0d, "b" -> CypherNull))
    ),
